...
class SumEmbeddingsModel(TfidfModel):
    ...
    def transform(self, samples, labels):
        x = np.array([np.sum(np.array([self.word_embeddings[j] for j in word_tokenize(i)], dtype=np.float),axis=0) for i in samples])
        self.x_train, self.x_test, self.y_train, self.y_test = train_test_split(x,labels, random_state=1, test_size=0.2)
